...
<form action="{{ route('curhat-baru.create') }}" method="POST">
            ...
            <div class="flex flex-col bg-white border shadow-sm rounded-xl pointer-events-auto">
                <div class="flex justify-between items-center py-3 px-4 border-b">
                    <h3 id="hs-slide-up-animation-modal-label" class="font-bold text-gray-800">
                        Cari Kategori - Pilih maximal 4 yaa
                    </h3>
                ...
                <div class="p-4 overflow-y-auto">
                    <div id="categories-list">
                        ...
                        <div name="category_id"
                            class="category-item py-2 px-4 inline-flex items-center gap-x-2 text-xs font-medium rounded-2xl border border-gray-400 bg-white text-black focus:outline-none focus:bg-gray-100 disabled:opacity-50 hover:cursor-pointer"
                            data-category="{{ $category->name }}" value="{{ $category->id }}">
                            {{ $category->name }}
                        </div>
                        @empty
    ...
    <div class="flex flex-col justify-center items-center">
        @error('category_id')
        <div class="block text-red-600 w-full md:w-3/4 lg:w-1/2">
            {{ $message }}
        </div>
        @enderror
    </div>
    ...
    <div class="py-8 flex justify-center items-center">
        <x-button-submit color="bg-secondary-700 text-white hover:bg-secondary-500 font-comfortaa">
            Kirim
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function updateSelectedCategories() {        
            ...
            selectedCategories.forEach(category => {
                ...
                categoryButton.setAttribute('id', `category-${category.name}`);
                ...
                categoryButton.innerHTML = `${category.name} <svg xmlns="http://www.w3.org/2000/svg" class="cursor-pointer remove-category" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
                        <path d="M18 6 6 18"></path>
            ...
            selectedCategories.forEach(category => {
                ...
                categoryWrapper.appendChild(input);
            });
        }
